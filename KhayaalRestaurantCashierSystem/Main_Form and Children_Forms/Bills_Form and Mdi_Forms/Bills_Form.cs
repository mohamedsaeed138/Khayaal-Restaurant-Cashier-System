...
namespace Khayaal.Main_Form_and_Children_Forms.Bills_Form_and_Mdi_Forms
{
    public partial class Bills_Form : Form
    {
        ...
        public Bills_Form(bool cashier = false)
        {
            Cashier = cashier;
            Reload();
        ...
        void Fill_Table(string Query)
        {
            ...
            Sum_Without_Tax_Value_Label.Text = Formatter.Float($"{Bills_Table.Rows.Cast<DataGridViewRow>().Sum(t => Convert.ToDouble(t.Cells[3].Value))}") + " $";
            Sum_Total_Value_Label.Text = Formatter.Float($"{Bills_Table.Rows.Cast<DataGridViewRow>().Sum(t => Convert.ToDouble(t.Cells[4].Value))}") + " $";
        ...
        void Choose_Query()
        {
            ...
            else
            {
                ...
                string Serial = Formatter.SpecialFormatting(Search_Serial_Number_Text_Box.Text);
                ...
                if (Total_Search_Text_Box.Text == "" && Search_Serial_Number_Text_Box.Text == "")
                    ...
                    Fill_Table($"select Serial_Number, Cashier_User_Name, Total, Date,[Cashier_User_Id] FROM CR.Bills WHERE [Serial_Number] LIKE '{Serial}%' AND Date BETWEEN '{From}' and '{To}' ;");
                else if (Total_Search_Text_Box.Text != "" && Search_Serial_Number_Text_Box.Text == "")
                    ...
                    Fill_Table($"select Serial_Number, Cashier_User_Name, Total, Date,[Cashier_User_Id] FROM CR.Bills WHERE [Total] >= {Total} AND [Serial_Number] LIKE '{Serial}%' AND Date BETWEEN '{From}' and '{To}' ;");
            }
        }
